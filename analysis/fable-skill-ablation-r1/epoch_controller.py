#!/usr/bin/env python3
"""Fail-closed single-writer primitives for one locked Fable ablation epoch.

This module is Card A of a two-card runtime slice: the offline controller
substrate a Card B runner/executor will compose against.  It deliberately
separates orchestration authority from provider execution.  The production
entry point acquires the persistent external lock before it reads a
manifest.  Provider processes can only be launched through the sandboxed
helper, after a durable provider-start intent, and never inherit the
controller's lock descriptor.
...
exercised by deterministic offline tests.  This card does not connect
runner/executor composition, does not implement full v3 execution
projection or a full epoch driver, and performs no real provider/model,
epoch-root, reservation, or spend write.
"""
...
def _load_usd_authority() -> Any:
    """Resolve the one canonical persisted USD and v3 manifest authority.

    The sibling compiler (``build_epoch_manifest.py``) owns manifest schema
    validation and the canonical fixed-seven persisted USD representation.
    Resolving it by module object -- rather than re-deriving a formatter or
    a second schema validator here -- keeps exactly one implementation of
    both across the runtime.
    """
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"canonical manifest/USD authority missing: {path}")
...
# and entry hashes stay valid forever and are never rewritten; only NEW
# writes use version 2 with the canonical fixed-seven persisted USD
# representation owned by build_epoch_manifest.py.
...
class ManifestAuthorityError(EpochControllerError):
    """The canonical compiler rejected the locked manifest bytes.

    Raised instead of a locally reinvented schema check: this module must
    never validate one byte sequence against its own rules and then act on
    a manifest the canonical compiler would have refused.
    """
...
class EpochLock:
    ...
    After successful acquisition the descriptor remains owned for the life
    of the controller process and is released by the kernel at process
    exit -- including on SIGKILL, so a crashed writer never leaves a stale
    lock that must be manually cleaned up.
    """
...
def _exact_amount(value: Decimal | str | int) -> Decimal:
    ...
    Cap and slot invariants are evaluated on this exact value so a
    sub-quantum amount can never slip under a limit by being rounded first.
    """
...
class BudgetLedger:
    ...
    def reserve_pair(
        self,
        arm_slot_ids: tuple[str, str],
        per_arm_cap: Decimal | str | int,
        pair_reservation_total: Decimal | str | int,
    ) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        """Reserve both arms of a pair together, or reserve neither.

        Before either arm of a pair may become launch-eligible, the whole
        pair must fit under the manifest-authorized cap.  A one-arm-first
        reservation that later discovers the second arm cannot fit would
        strand that second arm; this checks whole-pair capacity before
        either arm's reservation is durably written, and rolls the first
        arm back if writing the second unexpectedly fails.
        """

        if len(arm_slot_ids) != 2 or len(set(arm_slot_ids)) != 2:
            raise LedgerError("pair_reservation_requires_two_distinct_slots")
        exact_per_arm = _exact_amount(per_arm_cap)
        exact_pair_total = _exact_amount(pair_reservation_total)
        if exact_per_arm * 2 != exact_pair_total:
            raise LedgerError("pair_reservation_total_inconsistent_with_per_arm_cap")

        snapshot = self.load()
        for slot_id in arm_slot_ids:
            if slot_id in snapshot.used_slots:
                raise LedgerError("slot_already_consumed_no_silent_rerun")
        projected = (
            snapshot.lifetime_total_spend + snapshot.outstanding_total + exact_pair_total
        )
        if projected > self.lifetime_hard_cap:
            raise LedgerError("pair_reservation_would_strand_second_arm")

        first_slot, second_slot = arm_slot_ids
        first_record = self._append("reservation", first_slot, exact_per_arm)
        try:
            second_record = self._append("reservation", second_slot, exact_per_arm)
        except LedgerError:
            self._append("release", first_slot, exact_per_arm)
            raise
...
def budget_ledger_from_manifest(
    path: str | os.PathLike[str], manifest: Mapping[str, Any]
) -> BudgetLedger:
    """Open the one ledger a validated manifest authorizes.

    The manifest's own ``max_additional_spend_usd`` -- the lifetime hard cap
    minus the already-resolved historical charge -- becomes this ledger's
    effective cap.  Constructing a fresh ledger against the full nominal
    ``lifetime_hard_cap_usd`` instead would silently forget every dollar
    already spent under a prior epoch: a brand-new, empty ledger file must
    never behave like a brand-new budget.  When the historical charge is
    unresolved the manifest carries no usable remaining-budget figure at
    all, and this fails closed rather than inventing one.
    """

    budget = manifest.get("budget")
    if not isinstance(budget, Mapping):
        raise LedgerError(_error(AMBIGUOUS_LEDGER, "manifest_budget_missing"))
    if budget.get("budget_authority_status") != "AUTHORIZED_BY_EXPLICIT_OWNER_ATTESTATION":
        raise LedgerError(_error(AMBIGUOUS_LEDGER, "budget_authority_unresolved"))
    remaining = budget.get("max_additional_spend_usd")
    if not _usd.is_canonical_usd(remaining):
        raise LedgerError(_error(AMBIGUOUS_LEDGER, "max_additional_spend_usd_not_canonical"))
    return BudgetLedger(path, _usd.parse_canonical_usd(remaining))
...
def reconcile_fresh_start(
    *,
    ...
    This is a decision function only: it never kills a process, deletes a
    file, or retries anything.  The caller supplies every piece of live
    evidence (its own scan of the managed roots, captured process records,
    the ledger snapshot); Card A owns only the fail-closed judgment over
    that evidence, not how it was gathered.
    """
...
class ObservedSandboxIdentity:
    ...
    ``sandbox-exec`` binary and its bytes, the resolved profile and the
    SHA256 of its actual bytes, the bound SESSION_ROOT, and the physical
    argv that will really be executed.
    """
...
def start_reserved_provider(
    ...
    **spawn_options: Any,
    ...
    ``ledger.provider_start_intent`` either raises (no reservation, an
    already-started slot, or a cap mismatch) or durably appends the start
    intent before this function ever reaches ``spawn_sandboxed_provider``.
    A failed intent therefore always yields exactly zero spawn calls.
    """
...
def load_manifest_after_lock(
    ...
    *,
    ...
    """Hash, validate, and parse one securely opened snapshot after EpochLock.

    Exactly one inode-pinned descriptor is opened and read once; the same
    raw bytes are what gets hashed and what gets validated, so a manifest
    swapped in after this read can never be silently substituted.  Schema
    and signoff-authority validation is delegated entirely to the sibling
    compiler's ``validate_manifest_bytes``: this module must never parse or
    trust a manifest through a second, locally reinvented authority.
    """
    ...
    # Both operations consume this exact byte snapshot.  Reopening the
    # pathname could authenticate different content from the JSON used as
    # authority.
    observed_sha256 = hashlib.sha256(raw).hexdigest()
    if expected_sha256 is not None and observed_sha256 != expected_sha256:
        raise LifecycleError("authority_digest_mismatch")
    try:
        manifest = _usd.validate_manifest_bytes(raw)
    except _usd.ManifestValidationError as exc:
        raise ManifestAuthorityError(f"manifest_authority_rejected:{exc}") from exc
    return manifest
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        # Emit only a structural result; this card never starts a real
        # provider or materializes a real epoch through the command-line
        # entry point.
        print(
            json.dumps(
