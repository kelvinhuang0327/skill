...
name: fable-method
description: >-
  Top-level execution discipline for state-changing Worker tasks in
  Antigravity. Verify the Planner Packet against the real repository, choose
  one route (FAST, STANDARD, STANDARD_JUDGED, or LOOP_JUDGED), execute with an
  ...
  for a non-trivial task without a more specific skill. Antigravity
  enforcement is prompt-only; no hook mechanically enforces the gates. Works
  with task-specific or domain skills; this skill owns scope, routing,
  verification, retries, and closure. Subcommands: plan, audit, report.
---
...
For task-owned expensive / long-running launches, use the protected `--run` [entrypoint](references/task-checkpoint.md#protected-run-entrypoint) whenever runnable and preserve its stable record root and caller-supplied IDs; consider direct-local fallback only under [bounded launcher fallback](references/task-checkpoint.md#bounded-launcher-fallback). A selected-launcher failure or harness/platform permission denial never permits fallback. After a nonzero or ambiguous remote mutation result, allow one read-only observation of the exact authorized target under [post-attempt remote observation](references/task-checkpoint.md#post-attempt-remote-observation); it grants no retry permission or production-recovery status.
